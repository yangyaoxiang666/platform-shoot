...
public class Player1Bullet : MonoBehaviour
{
    ...
    void CheckComponents()
    {
        ...
        Debug.Log($"[{gameObject.name}] Player1射击器初始化完成。");
    }
    ...
    void Update()
    {
        ...
        {
            facingRight = false;
            Debug.Log($"[{gameObject.name}] Player1转向左侧");
        }
        if(Input.GetKeyDown(KeyCode.D))
        {
            facingRight = true;
            Debug.Log($"[{gameObject.name}] Player1转向右侧");
        }
    }
    ...
    void Shoot()
    {
        ...
        try
        {
            ...
            if (bulletCollision != null)
            {
                ...
                Debug.Log($"[{gameObject.name}] Player1设置子弹发射者为: {this.gameObject.name}");
            }
            ...
            if (rb2D != null)
            {
                ...
                Debug.Log($"[{gameObject.name}] Player1使用2D物理发射子弹！速度: {bullspeed}, 方向: {directionText}");
            }
            else
            {
                ...
                string directionText = facingRight ? "右" : "左";
                Debug.Log($"[{gameObject.name}] Player1使用3D物理发射子弹！速度: {bullspeed}, 方向: {directionText}");
            }
        ...
        {
            Debug.LogError($"[{gameObject.name}] Player1射击时发生错误: {e.Message}");
        }
    }
} 