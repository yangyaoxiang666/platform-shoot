...
public class Bullet : MonoBehaviour
{
    ...
    void CheckComponents()
    {
        ...
        Debug.Log($"[{gameObject.name}] 子弹射击器初始化完成。");
    }
    ...
    void Update()
    {
        ...
        {
            facingRight = false;
            Debug.Log($"[{gameObject.name}] 转向左侧");
        }
        if(Input.GetKeyDown(KeyCode.D))
        {
            facingRight = true;
            Debug.Log($"[{gameObject.name}] 转向右侧");
        }
    }
    ...
    void Shoot()
    {
        ...
        try
        {
            ...
            if (bulletCollision != null)
            {
                ...
                Debug.Log($"[{gameObject.name}] 设置子弹发射者为: {this.gameObject.name}");
            }
            ...
            if (rb2D != null)
            {
                ...
                Debug.Log($"[{gameObject.name}] 使用2D物理发射子弹！速度: {bullspeed}, 方向: {directionText}, 向量: {direction}");
            }
            else
            {
                ...
                string directionText = facingRight ? "右" : "左";
                Debug.Log($"[{gameObject.name}] 使用3D物理发射子弹！速度: {bullspeed}, 方向: {directionText}, 向量: {direction}");
            }
        ...
        {
            Debug.LogError($"[{gameObject.name}] 射击时发生错误: {e.Message}");
        }
    }
}
